...
## Purpose
Publish the package to npm safely and reproducibly from CI: triggered by published GitHub Releases (with a manual recovery path), gated so a misnamed tag can never publish the wrong version, routing prereleases to a side dist-tag, authenticating over OIDC-ready permissions, and installing build-correctly without git hooks.
